...
(20 h 45 – minuit) sur les chaînes de la TNT française**.
...
2. **Sélection** : on ne garde que la fenêtre « ce soir » (20 h 45 → minuit), et
   ...
   bandes-annonces) sont ignorés au moment de choisir.
...
4. **Rendu** (`utils/render.py`) : `dist/` est recréé à partir de `static/`
   ...
   `dist/data/epg.json` sont écrits de façon atomique.
